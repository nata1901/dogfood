    ...
    const updLike = (e) => {
        ...
        .then(res => res.json())
            .then(data => {
                ...
                // Если внутри функции set... (useState) находится callback, то его аргументом 
                // является старое состояние (то, что было до изменения) - в нашем случае мы назвали его "old"
                // callback обязательно должен вернуть новые данные
                setServerGoods(function(old) {
                    console.log(old)
                    // Нам надо из массива взять одну карточку и заменить ее. 
                    // При этом, положение карточки в массиве не должно поменяться. Наилучший способ пройтись по массиву, 
                    // изменив часть его - это метод map
